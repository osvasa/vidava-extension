...
function tryFindTotal(attempt) {
  ...
  for (var i = 0; i < lines.length; i++) {
    if (/est\.?\s*total|^total$/i.test(lines[i])) {
      ...
      if (sameLine) {
        ...
        if (v >= 1 && v <= 9999) {
          console.log('[VIDAVA] ' + label + ' — found on total line: $' + v.toFixed(2) + ' ("' + lines[i] + '")');
          return v;
        }
      }
      // Check the next line
      if (i + 1 < lines.length) {
        ...
        if (nextLine) {
          ...
          if (v2 >= 1 && v2 <= 9999) {
            console.log('[VIDAVA] ' + label + ' — found on line after total: $' + v2.toFixed(2) + ' ("' + lines[i + 1] + '")');
            return v2;
          }
        }
      }
    ...
    if (/^(est\.?\s*total|total)\s*$/i.test(elText) || /est\.?\s*total\s*(before|after)?/i.test(elText)) {
      ...
      for (var k = 0; k < searchEls.length; k++) {
        ...
        if (priceMatch) {
          ...
          if (pv >= 1 && pv <= 9999) {
            console.log('[VIDAVA] ' + label + ' — found via DOM walk near "' + elText + '": $' + pv.toFixed(2));
            return pv;
          }
        }
      }
  ...
  console.log('[VIDAVA] ' + label + ', prices found:', allPrices.map(function(p) { return '$' + p.toFixed(2); }));

  if (allPrices.length > 0) {
    var largest = Math.max.apply(null, allPrices);
    console.log('[VIDAVA] ' + label + ' — using largest price: $' + largest.toFixed(2));
    return largest;
  }

  console.log('[VIDAVA] ' + label + ' — no prices found');
  return null;
}
...
async function analyze() {
  ...
  // Load cards
  var data = await browser.storage.local.get(['vidava_cards']);
  var cards = data.vidava_cards;
  console.log('[VIDAVA overlay] Read vidava_cards from storage:', cards ? cards.length + ' cards' : 'none');
  ...
  // Debug: log full card data to verify details are present
  console.log('[VIDAVA] cards loaded for recommendation:', JSON.stringify(cards));
  ...
  var prompt =
    ...
    '- Bullet 1 (REQUIRED): APR comparison — "Your [card] has the lowest APR at X% — on this $XX purchase carried for 12 months you pay $XX total vs $XX on your next best card [name]"\n' +
    ...
    '- Never use jargon without explaining it simply. Every bullet must be specific and comparative.\n' +
    ...
    '{"best":{"name":"<exact card name>","bank":"<bank>","rate":"<e.g. 2% cash back>","value":"<dollar value e.g. $2.26>","reasons":["<bullet 1: APR comparison>","<bullet 2: rewards comparison>","<bullet 3: due date warning if relevant>","Bottom line: <true cost framing>"],"estimatedApr":"<user exact APR if provided, otherwise typical range e.g. 21.49%-29.49%>","aprWarning":"<warning about interest cost, or null>"},' +
    '"warnings":[{"card":"<name>","text":"<warning>"}],' +
    '"savings":"<best card reward dollar value>","category":"<detected category>"}';

  try {
    var resp = await browser.runtime.sendMessage({ type: 'ASK_AI', prompt: prompt });
        ...
        if (newAprNum && newAprNum > 0) {
          browser.storage.local.get('vidava_cards').then(function(result) {
            ...
            if (cardIndex !== -1) {
              ...
              browser.storage.local.set({ vidava_cards: storageCards }).then(function() {
                console.log('[VIDAVA] APR saved to storage for ' + recommendedCardName + ': ' + newAprNum + '%');
              });
            }
          });
...
function waitForReadyAndPrice() {
  browser.storage.local.get(['vidava_cards']).then(function(data) {
    if (!data.vidava_cards || data.vidava_cards.length === 0) return;

    // Show pill immediately so user knows VIDAVA is present
    pillWrap.style.display = 'block';
    console.log('[VIDAVA] pill shown, waiting for page load + price...');
    ...
    function tryAndOpen(source) {
      ...
      if (document.readyState !== 'complete') {
        console.log('[VIDAVA] ' + source + ' — page not complete yet (readyState: ' + document.readyState + ')');
        return;
      }
      ...
      if (found && found > 1) {
        resolved = true;
        detectedTotal = found;
        console.log('[VIDAVA] price confirmed: $' + found.toFixed(2) + ' — waiting 1s to settle...');
        // Wait 1 second for page to settle, then expand and analyze
        setTimeout(function() {
          console.log('[VIDAVA] expanding panel and starting analysis');
          open();
          analyze();
        }, 1000);
      }
    ...
    var interval = setInterval(function() {
      ...
      if (attempts > maxAttempts) {
        clearInterval(interval);
        if (observer) observer.disconnect();
        console.log('[VIDAVA] gave up after 15 seconds — expanding with no total');
        detectedTotal = null;
        open();
        analyze();
        return;
    ...
    if (document.readyState !== 'complete') {
      window.addEventListener('load', function() {
        console.log('[VIDAVA] page load event fired');
